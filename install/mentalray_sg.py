...
if DRQUEUE_OS == "WINDOWS":
	...
	SCENE = subprocess.Popen("cygpath.exe -w "+SCENE, shell=True, stdout=subprocess.PIPE).communicate()[0]
	RENDERDIR = subprocess.Popen("cygpath.exe -w "+RENDERDIR, shell=True, stdout=subprocess.PIPE).communicate()[0]
...
if RENDER_TYPE == "single image":
	...
	command = ENGINE_PATH+" "+repr(SCENE.strip())+" -render "+str(DRQUEUE_FRAME)+" "+str(BLOCK)
