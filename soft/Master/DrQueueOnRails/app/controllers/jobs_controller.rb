...
ENV['WEB_PROTO']+"://")
  ...
  def create
	...
	# MentalRay Standalone renderer	
	   	...
	   	# possible errors
	   	if scenefile == -1
			flash[:notice] = '没有找到可渲染的场景文件，请确保任务中有mi文件存在.'
	   		redirect_to :action => 'new' and return
		elsif scenefile == -2
			flash[:notice] = '没有找到场景文件，请检查压缩包'
	   		redirect_to :action => 'new' and return
	   	end
   		...
   		`sed -i 's/filename ".*\\\\/filename "/g' #{scenefile}_temp`
   		`sed -i '/clip true/d' #{scenefile}_temp`
